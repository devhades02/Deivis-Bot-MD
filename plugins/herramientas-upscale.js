// plugins/herramientas-upscale.js
...
const apiUrl = 'https://sharpify-api.vercel.app/api/enhance/upscale';
...
const handler = async (m, { conn, usedPrefix, command }) => {
  ...
  try {
    ...
    console.error('[Upscale] Error descargando imagen:', e.message);
  }
  ...
  if (!imgBuffer) {
    ...
    return m.reply(
      ...
      `Responde a una imagen con *${usedPrefix}${command}* para aumentar su resolución.`
    );
  }
  ...
  try {
    ...
    if (!imgUrl) {
      console.error('[Upscale] Respuesta API:', JSON.stringify(data));
      throw new Error('La API no devolvió imagen. Intenta con otra foto.');
    }

    const caption = `✅ Imagen ampliada\n${global.wm || 'Deivis Bot'}`;
